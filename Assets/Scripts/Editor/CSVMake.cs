...
public class CSVMake : Editor
{
    ...
    private static readonly string ConstomPath = "Assets/Resources/Text/Table/";
    ...
    [MenuItem("GameTool/Build CSV Files")]
    private static void BuildCSVFile()
    {
        //For Test
       // LoadConfig();
    ...
    private static void MakeCsv(string soucepath)
    {
        ...
        try
        {
            ...
            foreach (var pair in _tables)
            {
                ...
                log += "[CSVMake] 表格生成成功：" + ConstomPath + path + "\n";
            }
        ...
        {
            Debug.LogError("[CSVMake] 表格解析错误："  + tableName + "  " +e);
        }
        finally
        {
            _tablePass.Clear();
